...
SELECT ss.ca_county,
       ss.d_year,
       ws.sales_q2 / NULLIF(ws.sales_q1, 0) AS web_q1_q2_increase,
       ss.sales_q2 / NULLIF(ss.sales_q1, 0) AS store_q1_q2_increase,
       ws.sales_q3 / NULLIF(ws.sales_q2, 0) AS web_q2_q3_increase,
       ss.sales_q3 / NULLIF(ss.sales_q2, 0) AS store_q2_q3_increase
FROM ss_pivoted ss
...
WHERE ws.sales_q1 > 0
  ...
  AND ws.sales_q2 > 0
  AND ss.sales_q2 > 0
  AND (ws.sales_q2 / NULLIF(ws.sales_q1, 0)) > (ss.sales_q2 / NULLIF(ss.sales_q1, 0))
  AND (ws.sales_q3 / NULLIF(ws.sales_q2, 0)) > (ss.sales_q3 / NULLIF(ss.sales_q2, 0))
