...
SELECT count(ws_order_number) AS "order count",
       sum(total_ship_cost) AS "total shipping cost",
       sum(total_net_profit) AS "total net profit"
FROM
  (SELECT ws_order_number,
   ...
   HAVING count(DISTINCT ws_warehouse_sk) > 1) mws
WHERE NOT EXISTS
    (SELECT 1
     FROM web_returns wr
