WITH avg_discount_per_item AS
  (SELECT ws.ws_item_sk,
   ...
   GROUP BY ws.ws_item_sk),
     candidate_items AS
  (SELECT i.i_item_sk,
          ad.avg_discount_threshold
   FROM item i
   JOIN avg_discount_per_item ad ON i.i_item_sk = ad.ws_item_sk
...
FROM web_sales ws
JOIN candidate_items ci ON ws.ws_item_sk = ci.i_item_sk
JOIN date_dim d ON ws.ws_sold_date_sk = d.d_date_sk
WHERE d.d_date BETWEEN '2002-02-11' AND cast('2002-02-11' AS date) + interval '90 day'
  AND ws.ws_wholesale_cost BETWEEN 68 AND 88
  AND ws.ws_ext_discount_amt > ci.avg_discount_threshold
