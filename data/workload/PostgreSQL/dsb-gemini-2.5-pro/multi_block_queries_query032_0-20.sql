WITH avg_discount_per_item AS
  ...
  (SELECT cs_item_sk,
   ...
   JOIN date_dim ON cs_sold_date_sk = d_date_sk
   WHERE d_date BETWEEN '1999-01-14' AND cast('1999-01-14' AS date) + interval '90 day')
SELECT sum(ms.cs_ext_discount_amt) AS "excess discount amount"
FROM main_sales ms
JOIN item i ON ms.cs_item_sk = i.i_item_sk
...
WHERE (i.i_manufact_id IN (117,
  ...
  AND ms.cs_ext_discount_amt > adpi.threshold
ORDER BY sum(ms.cs_ext_discount_amt)
LIMIT 100;