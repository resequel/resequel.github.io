WITH filtered_web_returns AS
  ...
  (SELECT fwr.wr_returning_customer_sk AS ctr_customer_sk,
          ca.ca_state AS ctr_state,
          sum(fwr.wr_return_amt) AS ctr_total_return
   FROM filtered_web_returns fwr
   JOIN customer_address ca ON fwr.wr_returning_addr_sk = ca.ca_address_sk
   GROUP BY fwr.wr_returning_customer_sk,
            ca.ca_state),
     state_avg_return AS
  (SELECT ctr_state,
...
WHERE ctr1.ctr_total_return > (ctr2.avg_return * 1.2)
