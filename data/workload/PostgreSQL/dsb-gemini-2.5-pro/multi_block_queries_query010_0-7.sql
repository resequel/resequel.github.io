...
LEFT JOIN web_customers wc ON c.c_customer_sk = wc.customer_sk
LEFT JOIN catalog_customers cc ON c.c_customer_sk = cc.customer_sk
WHERE ca.ca_county IN ('Audubon County',
  ...
  AND cd.cd_education_status IN ('Primary',
                              ...
                              '4 yr Degree')
  AND cd.cd_gender = 'M'
  AND (wc.customer_sk IS NOT NULL
       OR cc.customer_sk IS NOT NULL)
GROUP BY cd_gender,
         cd_marital_status,
         cd_education_status,
         cd_purchase_estimate,
