WITH customer_filtered AS
  ...
  (SELECT ss.ss_item_sk,
   ...
   JOIN customer_filtered c ON ss.ss_customer_sk = c.c_customer_sk),
     ws_filtered AS
  (SELECT ws.ws_item_sk,
   ...
   JOIN customer_filtered c ON ws.ws_bill_customer_sk = c.c_customer_sk
...
SELECT min(ss.ss_item_sk),
       ...
       min(hd.hd_demo_sk)
FROM ss_filtered ss
JOIN ws_filtered ws ON ss.ss_item_sk = ws.ws_item_sk
...
WHERE d2.d_date BETWEEN ss.d_date AND (ss.d_date + interval '30 day')
