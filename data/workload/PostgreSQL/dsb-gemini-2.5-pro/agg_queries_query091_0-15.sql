...
JOIN
  (SELECT d_date_sk
   FROM date_dim
   WHERE d_year = 1999
     AND d_moy = 5) d ON cr.cr_returned_date_sk = d.d_date_sk
JOIN
  (SELECT c.c_customer_sk
   ...
   WHERE ca.ca_gmt_offset = -7
     ...
     AND (cd.cd_marital_status,
          cd.cd_education_status) IN (('M', 'Unknown'), ('W', 'Advanced Degree'))) fc ON cr.cr_returning_customer_sk = fc.c_customer_sk
