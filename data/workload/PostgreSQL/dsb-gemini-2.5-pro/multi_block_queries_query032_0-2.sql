...
JOIN
  (SELECT i_item_sk
   ...
   WHERE i_manufact_id IN (117,
                         ...
                         849,
                         891)
     OR i_manager_id BETWEEN 28 AND 57) i ON cs.cs_item_sk = i.i_item_sk
JOIN
  (SELECT d_date_sk
   FROM date_dim
   WHERE d_date BETWEEN '1999-01-14' AND cast('1999-01-14' AS date) + interval '90 day') d ON cs.cs_sold_date_sk = d.d_date_sk
JOIN avg_discount ad ON cs.cs_item_sk = ad.cs_item_sk
WHERE cs.cs_ext_discount_amt > ad.threshold
