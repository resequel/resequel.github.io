WITH inv_month1 AS
  (SELECT inv.inv_warehouse_sk AS w_warehouse_sk,
   ...
   AND (stddev_samp(inv.inv_quantity_on_hand) / NULLIF(avg(inv.inv_quantity_on_hand), 0)) > 1.5),
     inv_month2 AS
  (SELECT inv.inv_warehouse_sk AS w_warehouse_sk,
          inv.inv_item_sk AS i_item_sk,
          d.d_moy,
...
SELECT inv1.w_warehouse_sk,
       ...
       inv2.mean,
       inv2.cov
FROM inv_month1 inv1
JOIN inv_month2 inv2 ON inv1.i_item_sk = inv2.i_item_sk
