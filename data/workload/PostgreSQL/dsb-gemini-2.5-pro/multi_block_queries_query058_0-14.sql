WITH month_seqs AS
  ...
  (SELECT d_date_sk
   FROM date_dim,
        month_seqs
   WHERE d_month_seq = ss_m_seq),
     cs_dates AS
  (SELECT d_date_sk
   FROM date_dim,
        month_seqs
   WHERE d_month_seq = cs_m_seq),
     ws_dates AS
  (SELECT d_date_sk
   FROM date_dim,
        month_seqs
   WHERE d_month_seq = ws_m_seq),
     ss_filtered_items AS
  (SELECT i_item_sk,
          i_item_id
   FROM item
  ...
  (SELECT i.i_item_id AS item_id,
   ...
   FROM store_sales ss
   JOIN ss_dates d ON ss.ss_sold_date_sk = d.d_date_sk
  ...
  (SELECT i.i_item_id AS item_id,
   ...
   JOIN cs_dates d ON cs.cs_sold_date_sk = d.d_date_sk
  ...
  (SELECT i.i_item_id AS item_id,
   ...
   FROM web_sales ws
   JOIN ws_dates d ON ws.ws_sold_date_sk = d.d_date_sk
