WITH return_days_calc AS
  (SELECT ss.ss_store_sk,
   ...
   WHERE d2.d_year = 2000
     ...
     store_level_returns AS
  (SELECT rdc.ss_store_sk,
          sum(CASE
                  WHEN (rdc.return_days <= 30) THEN 1
                  ELSE 0
              END) AS days_30,
          sum(CASE
                  WHEN (rdc.return_days > 30)
                       AND (rdc.return_days <= 60) THEN 1
                  ELSE 0
              END) AS days_60,
          sum(CASE
                  WHEN (rdc.return_days > 60)
                       AND (rdc.return_days <= 90) THEN 1
                  ELSE 0
              END) AS days_90,
          sum(CASE
                  WHEN (rdc.return_days > 90)
                       AND (rdc.return_days <= 120) THEN 1
                  ELSE 0
              END) AS days_120,
          sum(CASE
                  WHEN (rdc.return_days > 120) THEN 1
                  ELSE 0
              END) AS days_over_120
   FROM return_days_calc rdc
   GROUP BY rdc.ss_store_sk)
