WITH filtered_dims_ss AS
  ...
  (SELECT s.s_store_id AS store_id,
   ...
   FROM store_sales ss
   JOIN filtered_dims_ss f ON ss.ss_sold_date_sk = f.d_date_sk
   AND ss.ss_item_sk = f.i_item_sk
   AND ss.ss_promo_sk = f.p_promo_sk
  ...
  (SELECT cp.cp_catalog_page_id AS catalog_page_id,
   ...
   JOIN filtered_dims_cs f ON cs.cs_sold_date_sk = f.d_date_sk
   AND cs.cs_item_sk = f.i_item_sk
   AND cs.cs_promo_sk = f.p_promo_sk
  ...
  (SELECT w.web_site_id,
   ...
   FROM web_sales ws
   JOIN filtered_dims_ws f ON ws.ws_sold_date_sk = f.d_date_sk
   AND ws.ws_item_sk = f.i_item_sk
   AND ws.ws_promo_sk = f.p_promo_sk
