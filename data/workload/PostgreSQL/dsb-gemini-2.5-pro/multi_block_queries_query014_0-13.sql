WITH avg_sales AS
  ...
  (SELECT DISTINCT i_brand_id,
   ...
   WHERE d3.d_year BETWEEN 1998 AND 1998 + 2
     AND ws_wholesale_cost BETWEEN 80 AND 100),
     common_bci AS
  (SELECT ss_bci.i_brand_id,
          ss_bci.i_class_id,
          ss_bci.i_category_id
   FROM ss_bci
   JOIN cs_bci ON ss_bci.i_brand_id = cs_bci.i_brand_id
   AND ss_bci.i_class_id = cs_bci.i_class_id
   AND ss_bci.i_category_id = cs_bci.i_category_id
   JOIN ws_bci ON ss_bci.i_brand_id = ws_bci.i_brand_id
   AND ss_bci.i_class_id = ws_bci.i_class_id
   AND ss_bci.i_category_id = ws_bci.i_category_id),
     cross_items AS
  (SELECT i_item_sk
   FROM item i
