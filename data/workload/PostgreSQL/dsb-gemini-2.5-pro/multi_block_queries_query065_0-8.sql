WITH item_revenues AS
  (SELECT ss.ss_store_sk,
          ss.ss_item_sk,
          SUM(CASE
                  WHEN d.d_month_seq BETWEEN 1215 AND 1215+11 THEN ss.ss_sales_price
              END) AS revenue_a,
          SUM(CASE
                  WHEN d.d_month_seq BETWEEN 1215 AND 1215+11 THEN ss.ss_sales_price
              END) AS revenue_c
   ...
   WHERE d.d_month_seq BETWEEN LEAST(1215, 1215) AND GREATEST(1215+11, 1215+11)
     AND ss.ss_sales_price BETWEEN ss.ss_list_price * LEAST(22 * 0.01, 22 * 0.01) AND ss.ss_list_price * GREATEST(32 * 0.01, 32 * 0.01)
